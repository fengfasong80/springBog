...
$.ajax({
	type : "POST",
	url : base + "/commerce/goodType/list",
	dataType : 'json',
	async: false,
...
$('#addGoodTypeCombox').combobox({
    valueField:'id',
    textField:'name',
    data: goodTypeDict
});

$('#updateGoodTypeCombox').combobox({
    valueField:'id',
    textField:'name',
    data: goodTypeDict
});
...
$('#goodGrid').datagrid({
    ...
    columns:[[
        ...
        {field:'goodTypeId',title:'商品类型',width:100,sortable:true,
	        	formatter:function(value,row,index){
	        		return dictValue(goodTypeDict, 'id', 'name', value)
	        }
        }
    ]]
});
...
function queryGood() {
	var param = {};
	generateCondition(param, $('#queryGoodForm').serialize());
	$("#goodGrid").datagrid("reload",param)
};